...
import { Cell, checkIfNeighbourExists, Configuration, getNeighbours, processTurn, removeDuplicatesNeighbours } from '../core/coreFunctions';
...
describe('Process a turn', function () {
    ...
    it('Represent a glider pattern of living cells', function () {
        let config: Configuration = [
            { x: 5, y: 5 },
            { x: 6, y: 6 },
            { x: 6, y: 7 },
    ...
    it('Represent swing pattern of living cells, moving from | to - permanently', function () {
        let config: Configuration = [
            { x: 41, y: 41 },
            { x: 42, y: 41 },
            { x: 43, y: 41 },
        ];
        let swing1: Configuration = processTurn(config);
        let swing2: Configuration = processTurn(swing1);
        expect(swing2).toIncludeAllMembers(config);
    });
});